...
public class SearchInRotatedSortedArray {
	...
	private static int search(int[] A, int target) {
		...
		while(low<=high) {
			...
			if(A[low]<=A[mid]) {// nums[start.......mid] is sorted
				if(A[low]<=target&&target<A[mid]) {// need to check target lies between array[start...mid]
					high=mid-1;
				}else {
					low=mid+1;
				}
			}else {// nums[mid.......end] is sorted
				if(target<=A[high]&&target>A[mid]) {//need to check target lies between array[mid...end]
					low=mid+1;
				}else {
					high=mid-1;
				}
