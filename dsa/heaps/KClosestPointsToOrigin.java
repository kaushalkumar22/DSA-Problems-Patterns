...
public class KClosestPointsToOrigin {
	...
	public int[][] kClosest1(int[][] points, int k) {
		...
		while(low <=high){
			...
			if(pivot>k){
				...
				res = Arrays.copyOfRange(points,0, pivot);
				break;
			}
		}
		return res;
	...
	private int partition(int[][] points,int low, int high){
		...
		for(int i =low ;i<high;i++){
			if(distance(points[i][0], points[i][1])<=pivot){
	...
	private int partition1(int[][] points, int low, int high) {
	...
	private void swap1(int[][] points, int i, int j) {
		int[] temp =points[i];
		points[i]=points[j];
		points[j]=temp;
	}
