...
Given k words, find top k frequent items. MinHeap must be used, but there are two options:
    ...
    Compute MinHeap on the fly: Of course, we canÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢t come to a DFS every time, otherwise it will be very slow, so we must store a list of possible candidates in TrieNode.
...
The problem is very long, but not too difficult, it can be solved with Trie, but you need to pay attention:
    ...
    When encounteringÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢#ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢, return to empty list
    ...
    In dfs trie, you must now add root to the result set, and then traverse the children! Otherwise, the root at the bottom of the stack will be missed. This is very important. TrieÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s queries must be written like this.
	...
	private static class Pair {
		String s;
		int count;
		public Pair(String s, int count) { this.s = s; this.count = count; }
	}
}
