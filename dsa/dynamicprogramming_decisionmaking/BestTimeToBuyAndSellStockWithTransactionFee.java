package dsa.dynamicprogramming_decisionmaking;
...
public class BestTimeToBuyAndSellStockWithTransactionFee {
	...
	public static int maxProfit(int[] prices, int fee) {
		...
		for (int price :prices){
			buy = Math.max(buy, sell - price - fee); // keep the same as day i-1, or buy from sell status at day i-1
			sell = Math.max(sell, buy + price); // keep the same as day i-1, or sell from buy status at day i-1
		}
		return sell;
	}
	...
	public static int maxProfit2(int[] prices, int fee) {
		...
		int buy =Integer.MIN_VALUE;
		for (int price :prices){
			buy = Math.max(buy, sell - price ); // keep the same as day i-1, or buy from sell status at day i-1
			sell = Math.max(sell, buy + price- fee); // keep the same as day i-1, or sell from buy status at day i-1
		}
		return sell;
	}
}
