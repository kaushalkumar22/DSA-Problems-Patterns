...
public class FlattenAMultilevelDoublyLinkedList {
	...
	public Node flatten(Node head) {
		...
		while(curr!=null) {
			...
			if(curr.child==null) {
				curr= curr.next;
			}else {/* CASE 2: got child, find the tail of the child and link it to curr.next */
				Node cChild =curr.child;
				while(cChild.next!=null) {// Find the tail of the child
					cChild=cChild.next;
				}			
				cChild.next = curr.next;
				// Connect tail with curr.next, if it is not null
				if(curr.next!=null) {
					curr.next.prev=cChild;
				}
