...
We make two passes over the strings:

First Pass: Count bulls and track digits (frequencies) from secret that are not bulls.

Second Pass: For non-bull positions in guess, if the digit exists in our map (tracked from secret), it’s a cow.
